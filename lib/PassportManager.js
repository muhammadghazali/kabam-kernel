...
//using default strategies
    ...
    passport.use(new GoogleStrategy({
      ...
      function (identifier, profile, done) {
        // TODO: add normal logging for this
        console.log('==============');
        console.log(profile);
        console.log('==============');
        ...
        if (email) {
          ...
          // TODO: we can use returned familyName and givenName to complete user profile
        } else {
  ...
  this.doInitializePassportRoutes = function (passport, core) {
    ...
    // TODO: test redirect
    core.app.get('/auth/google', function(req, res, next){
      var redirectTo;
      if(req.query.redirect) {
        redirectTo = url.resolve(core.config.hostUrl, req.query.redirect);
      }
      req.session.redirectTo = redirectTo || req.header('Referrer');
      req.session.referrer = req.header('Referrer');
      next();
    }, passport.authenticate('google'));

    core.app.get('/auth/google/return', function(req, res, next) {
      req.session.redirectTo || (req.session.redirectTo = '/');
      passport.authenticate('google', function(err, user, info) {
        if (err) {
          req.flush('error', err.message);
          // referrer has higher priority, since we want return to the login location
          return res.redirect(req.session.referrer || req.session.redirectTo);
        }
        if (!user) {
          return res.redirect(req.session.redirectTo);
        }
        req.logIn(user, function(err) {
          // serialization error - don't need to be shown to the user, just give them 500
