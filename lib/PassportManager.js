  ...
  this.doInitializePassportRoutes = function (passport, core) {
    ...
    core.app.get('/auth/success', function (request, response) {
      ...
      if (request.session.redirectTo) {
        var p = url.parse(request.session.redirectTo);
        if (p) {
          if (p.hostname === url.parse(core.config.hostUrl).hostname) { //catching redirect to other site
            response.redirect(url.format(p)); //this is valid sanitazed redirect
          } else {
            response.redirect('/');
          }
        } else { //redirect url does not have proper syntax, redirecting to top page
          response.redirect('/');
        }
      } else {
        response.redirect('/');
    ...
    core.app.get('/auth/failure', function (request, response) {
      ...
      if (request.session.redirectTo) {
        var p = url.parse(request.session.redirectTo);
        if (p) {
          if (p.hostname === url.parse(core.app.config.hostUrl).hostname) { //catching redirect to other site
            response.redirect(url.format(p)); //this is valid sanitazed redirect
          } else {
            response.redirect('/');
          }
