...
function processSchema (config, defaults) {
  ...
  walk(config, function (object, property) {
    ...
    // jshint expr: true
    disabledPlugins[err.pluginName] || (disabledPlugins[err.pluginName] = err);
  });
...
module.exports = function buildConfig(rawConfig, plugins) {
  ...
  // merging raw config and parsed config, this allows us to keep config variables that weren't define on any plugins
  config = _.merge(_.cloneDeep(rawConfig), config);
  return {
    config: config,
