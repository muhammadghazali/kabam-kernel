...
//    consumerKey: 'vSaltBeXXpz9UJ96Pzb0g',//core.config.passport.TWITTER_CONSUMER_KEY,
    consumerKey: core.config.passport.TWITTER_CONSUMER_KEY,
//    consumerSecret: 'SWVs4PsX948Vg5oJHUteV81A845fKxd7uoDdmeHL7cU',//core.config.passport.TWITTER_CONSUMER_SECRET,
    ...
    if(profile.provider === 'twitter' && profile.id){
      core.model.Users.findOneByKeychain('twitter',profile.id, function(err,userFound){
        if(err) {
          ...
          if(userFound){ //it works for now
            done(null,userFound);
          } else {
            done(null,profile); //it fails - inspect close of passport twitter more closely
          }
        }
      });
    } else {
...
exports.routes = function (passport, core) {
  ...
  core.app.get('/auth/twitter/callback', passport.authenticate('twitter', { failureRedirect: '/',successRedirect: '/auth/twitter/extend' }));

  core.app.get('/auth/twitter/extend',function (req, res) {
      //we need all this fiffles and fluffles to add Twitter id to users keychain.
      ...
      if(req.user.provider === 'twitter'){
        if(req.session.userToBeExtended){
          ...
          req.model.Users.findOneByLoginOrEmail(req.session.userToBeExtended,function(err,userFound){
            if(err) throw err;
            if(userFound){
              userFound.setKeyChain('twitter',req.user.id,function(err){
                if(err) throw err;
                req.flash('success','Twitter profile is attached to keychain!');
                req.user = userFound;
                res.redirect('/');
              });
            } else {
              req.flash('error','Unable to signin via Twitter.com! Please, register with username, email and password and than attach your github profile to it!');
              req.logout();
              res.redirect('/');
            }
          });
