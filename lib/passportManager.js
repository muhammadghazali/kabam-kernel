...
passportManager.doInitializePassportRoutes = function (passport, core) {
  ...
  core.app.post('/auth/signup', function (request, response) {
    var regularHandler = function (err, userCreated) {
        if (err) {
      ...
      jsonHandler = function (err, userCreated) {
        if(err){
          response.json(400, {errors: {form: err.message}});
          return;
        }
        //TODO: Maybe we need a way to get a save user profile in json?
        delete userCreated.password;
        delete userCreated.salt;
        delete userCreated.apiKey;
        response.json(userCreated);
      },
      handler = regularHandler;
    if(request.headers.accept.match(/application\/json/)) handler = jsonHandler;
    request.model.Users.signUp(request.body.username, request.body.email, request.body.password, handler);
  });
