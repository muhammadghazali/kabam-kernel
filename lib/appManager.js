...
module.exports = exports = function (mwc, _extendAppFunctions, _additionalStrategies, _extendMiddlewareFunctions, _extendRoutesFunctions) {
  ...
  mwc.app = express();
  if(mwc.config.io){
    var server = http.createServer(mwc.app);
    mwc.io = ioServer.listen(server);
...
//*/
    ...
    mwc.io.set('log level', (mwc.config.io.loglevel)?(mwc.config.io.loglevel):3);


    var ioRedisStore = require('socket.io/lib/stores/redis');

    mwc.set('store', new ioRedisStore({
      redis: ioRedis,
...
// console.log(io.sockets.manager.sockets.sockets[x]);
  ...
  if(!mwc.config.disableCsrf){
    ...
    mwc.app.use(function(request, response, next){
      if (request.session) {
        request.csrfToken(function(err,token){
