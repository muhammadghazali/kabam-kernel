...
module.exports = exports = function (mwc, _extendAppFunctions, _additionalStrategies, _extendMiddlewareFunctions, _extendRoutesFunctions, _catchAllFunction) {
  ...
  if (mwc.config.toobusy && mwc.config.toobusy.maxLag) {
    var maxLag = parseInt(mwc.config.toobusy.maxLag);
    if (typeof maxLag === 'number') {
      console.log('setting maxLag to ', maxLag);
      toobusy.maxLag(mwc.config.toobusy.maxLag);
    }
  }
    ...
    if (/^\/auth/.test(request.originalUrl)) { //we cannot make redirects back to /auth/* path
      ...
      if (request.user) {
        ...
        if (request.session.redirectTo) {
          var hashRedirect; // /someUrl#/someAngularUrl
          if (request.query && request.query.redirect) {
            hashRedirect = '#' + request.query.redirect;//todo  some sort of validation
          }
          request.session.redirectTo = request.originalUrl + hashRedirect;
        }
        next();
      }
    }
