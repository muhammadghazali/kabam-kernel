...
module.exports = exports = function (kabam, _extendAppFunctions, _additionalStrategies, _extendMiddlewareFunctions, _extendRoutesFunctions, _catchAllFunction) {
  ...
  if (kabam.config.toobusy && kabam.config.toobusy.maxLag) {
    var maxLag = parseInt(kabam.config.toobusy.maxLag);
    if (typeof maxLag === 'number') {
      console.log('setting maxLag to ', maxLag);
      toobusy.maxLag(kabam.config.toobusy.maxLag);
    }
  }
...
// console.log(io.sockets.manager.sockets.sockets[x]);
  ...
  kabam.app.set('port', process.env.PORT || 3000);
  ...
  kabam.app.use(rateLimiter(kabam.redisClient, ((kabam.config.rateLimit) ? (kabam.config.rateLimit) : 200)));//200 requests per minute for every IP
  ...
  kabam.app.use(function (request, response, next) {
    ...
    kabam.app.locals.hostUrl = kabam.config.hostUrl;
